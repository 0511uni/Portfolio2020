...
public class MovePlayer2 : MonoBehaviour {　// 長押しbutton操作で動かすスクリプト(プレイヤー)
...
//          プレイヤーの位置が6.0f以下の時
//          ↑画面からはみ出さないための条件
            transform.position += new Vector3 (9.0f * Time.deltaTime, 0, 0);
//          プレイヤーをx軸方向に秒速6.0fで動かす
            transform.rotation = Quaternion.Euler (0, 0, -50);
//          プレイヤーの角度をy軸周りに0度、z軸周りに-50度回転させる
...
//          プレイヤーの位置が-3.0f以上の時
//          ↑画面からはみ出さないための条件
            transform.position += new Vector3 (0 * Time.deltaTime, 0, 0.9f);
